...
public class DotaskExtractAdapter extends FragmentPagerAdapter {
    ...
    /*private ArrayList<DotaskExtractFragment> mFragmentList;
    private FragmentManager mFragmentManager;

    public DotaskExtractAdapter(FragmentManager fm, ArrayList<DotaskExtractFragment> list) {
        super(fm);
        this.mFragmentManager = fm;
        mFragmentList = new ArrayList<>();
        for (int i = 0, size = list.size(); i < size; i++) {
            mFragmentList.add(DotaskExtractFragment.newInstance(i));
        }
        setFragments(list);
    }

    public void updateData(List<Integer> dataList) {
        ArrayList<DotaskExtractFragment> fragments = new ArrayList<>();
        for (int i = 0, size = dataList.size(); i < size; i++) {
            Log.e("FPagerAdapter1", dataList.get(i).toString());
            fragments.add(DotaskExtractFragment.newInstance(dataList.get(i)));
        }
        setFragments(fragments);
    }

    private void setFragments(ArrayList<DotaskExtractFragment> mFragmentList) {
